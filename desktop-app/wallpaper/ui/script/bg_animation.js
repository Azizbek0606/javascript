...
// const rainSound = new Audio("../../assets/sounds/rain/rain.mp3");
// rainSound.loop = true;
// rainSound.volume = 0.5;

// const thunderSounds = [
//     new Audio("../../assets/sounds/thunder/first.mp3"),
//     new Audio("../../assets/sounds/thunder/second.mp3"),
//     new Audio("../../assets/sounds/thunder/third.mp3")
// ];
...
const clouds = Array.from({ length: 100 }, () => createCloud(
    Math.random() * 100 - 50,
    Math.random() * 10 + 15,
    Math.random() * 100 - 50
));
...
for (let i = 0; i < rainCount; i++) {
    ...
    rainPositions.set([x, y, z, x, y - 0.5, z], i * 6);
    velocities[i] = Math.random() * 0.4 + 0.3;
}

rainGeometry.setAttribute('position', new THREE.BufferAttribute(rainPositions, 3));
const rainMaterial = new THREE.LineBasicMaterial({
    color: 0x87CEEB, transparent: true, opacity: 0.7, linewidth: 1
});
const rain = new THREE.LineSegments(rainGeometry, rainMaterial);
scene.add(rain);
...
function animate() {
    ...
    for (let i = 0; i < rainCount; i++) {
        ...
        if (positions[i * 6 + 1] < -5) {
            const x = Math.random() * 60 - 30;
            const z = Math.random() * 60 - 30;
            positions.set([x, 20, z, x, 19.5, z], i * 6);
        }
    }
    rain.geometry.attributes.position.needsUpdate = true;
